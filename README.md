...
#导入alpine系统的容器镜像（默认的）
wget --no-check-certificate https://drive.yangwn.top/d/AliDrive/Linux/Docker/Zabbix/images/zabbix-image-5.0.tar.gz
tar xf zabbix-image-5.0.tar.gz
cd zabbix-image-5.0
./docker_load.sh
docker image ls -a

#导入centos系统的容器镜像（需要修改docker-compose.yaml的启动镜像）
wget --no-check-certificate https://drive.yangwn.top/d/AliDrive/Linux/Docker/Zabbix/images/zabbix-image-5.0-centos.tar.gz
tar xf zabbix-image-5.0-centos.tar.gz
cd zabbix-image-5.0-centos
./docker_load.sh
docker image ls -a
```
...
sed -i 's#alpine-5.0#centos-5.0#g' docker-compose.yaml
```
